...
while True:
        ...
        if not choice.isdigit() or int(choice) not in menu:
            print(RED + BOLD + "❌Invalid input! Please enter a valid input.")
            continue
        ...
        while True:
            ...
            if not quantity.isdigit() or int(quantity) <= 0:
                print(RED + BOLD + "❌Invalid quantity please enter a vaild input !.")
                continue
...
print(BG_WHITE+ f"{'=' * 50}")
print(BG_WHITE+f"{'GROCERY RECEIPT':^50}")
print(BG_WHITE+ f"{'=' * 50}")

print(BG_WHITE+ f" {'Store:    ABCD PLC':<49}")
print(BG_WHITE+ f" {'Location: Arat Killo Plaza':<49}")
print(BG_WHITE+ f" {'Machine:  VM-001':<49}")
print(BG_WHITE+"-" * 50)

print(BG_WHITE+f" Transaction #: {transaction_number:<34}")
print(BG_WHITE+f" Date & Time:   {date_time:<34}")
print(BG_WHITE+"-" * 50)

print(BG_WHITE+CYAN+ f" {'Item':<15}{'quantity':^15}{'price':<10}{'Subtotal':^9}")
print(BG_WHITE+"-" * 50)
total = 0
for number, quantity in selected_items.items():
    item, price = menu[number]
    line_total = price * quantity
    print(BG_WHITE + f"{item:<15}{quantity:^15}${price:<9.2f}${line_total:<8.2f}" + RESET)
    total += line_total
print(BG_WHITE+"-" * 50)
print(BG_WHITE+f" Total: {'$':>34}{total :<7.2f} ")
print(BG_WHITE+"=" * 50)
...
while True:
    try:
        ...
        if payment < total:
            print(RED + f"⚠️Insufficient payment! You still owe ${total - payment:.2f}." + RESET)
        else:
            loading_animation(0.5)
